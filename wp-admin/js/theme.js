...
(function ($) {
     ...
     themes.view.Appearance = wp.Backbone.View.extend({
          ...
          search: function () {
               ...
               this.searchContainer
                    ...
                    .append(
                         $.parseHTML(
                              '<label for="wp-filter-search-input">' +
                                   l10n.search +
                                   '</label>',
                         ),
                    )
     ...
     themes.view.Details = wp.Backbone.View.extend({
          ...
          containFocus: function ($el) {
               ...
               $el.on('keydown.wp-themes', function (event) {
                    var $firstFocusable = $el
                              ...
                              .first(),
                         $lastFocusable = $el
                              .find('.theme-actions a:visible')
                              .last();

                    // Check for the Tab key.
                    if (9 === event.which) {
                         if (
                              $firstFocusable[0] === event.target &&
                              event.shiftKey
                         ) {
                              $lastFocusable.trigger('focus');
                              event.preventDefault();
                         } else if (
                              $lastFocusable[0] === event.target &&
                              !event.shiftKey
                         ) {
          ...
          navigation: function () {
               ...
               if (
                    this.model.cid ===
                    this.model.collection.at(this.model.collection.length - 1)
                         .cid
               ) {
                    this.$el
                         .find('.right')
                         ...
                         $('[data-slug="' + response.slug + '"]')
                              ...
                              .fadeOut(350, function () {
                                   ...
                                   _themes.data.themes = _.without(
                                        _themes.data.themes,
                                        _.findWhere(_themes.data.themes, {
                                             id: response.slug,
                                        }),
                                   );
     ...
     themes.view.Preview = themes.view.Details.extend({
          ...
          render: function () {
               ...
               themes.router.navigate(
                    themes.router.baseUrl(
                         themes.router.themePath + this.model.get('id'),
                    ),
                    { replace: false },
               );
          ...
          close: function () {
               ...
               if (themes.router.selectedTab) {
                    themes.router.navigate(
                         themes.router.baseUrl(
                              '?browse=' + themes.router.selectedTab,
                         ),
                    );
     ...
     themes.view.Themes = wp.Backbone.View.extend({
          ...
          initialize: function (options) {
               ...
               $('body').on('keyup', function (event) {
                    ...
                    if (
                         $('#request-filesystem-credentials-dialog').is(
                              ':visible',
                         )
                    ) {
                         return;
          ...
          expand: function (id) {
               ...
               themes.router.navigate(
                    themes.router.baseUrl(
                         themes.router.themePath + this.model.id,
                    ),
               );
          ...
          next: function (args) {
               ...
               nextModel = self.collection.at(
                    self.collection.indexOf(model) + 1,
               );
          ...
          previous: function (args) {
               ...
               previousModel = self.collection.at(
                    self.collection.indexOf(model) - 1,
               );
     ...
     themes.view.Search = wp.Backbone.View.extend({
          ...
          doSearch: function (event) {
               ...
               if (event.target.value) {
                    themes.router.navigate(
                         themes.router.baseUrl(
                              themes.router.searchPath + event.target.value,
                         ),
                         options,
                    );
               } else {
     ...
     themes.view.Installer = themes.view.Appearance.extend({
          ...
          filtersChecked: function () {
               ...
               $('.filter-drawer .apply-filters')
                    .find('span')
                    .text(tags.length);
               $('.filter-drawer .clear-filters').css(
                    'display',
                    'inline-block',
               );
          ...
          routes: function () {
               ...
               themes.router.on('route:preview', function (slug) {
                    ...
                    if (self.view.view.theme && self.view.view.theme.preview) {
                         self.view.view.theme.model =
                              self.view.collection.findWhere({ slug: slug });
               ...
               themes.router.on('route:sort', function (sort) {
                    if (!sort) {
                         sort = 'popular';
                         themes.router.navigate(
                              themes.router.baseUrl('?browse=popular'),
                              { replace: true },
                         );
                    }
                    self.view.sort(sort);
