     ...
     ImageWidgetControl = component.MediaWidgetControl.extend(
          /** @lends wp.mediaWidgets.controlConstructors.media_image.prototype */ {
               ...
               renderPreview: function renderPreview() {
                    ...
                    previewContainer = control.$el.find(
                         '.media-widget-preview',
                    );
                    previewTemplate = wp.template(
                         'wp-media-widget-image-preview',
                    );
                    ...
                    if (!linkInput.is(document.activeElement)) {
                         fieldsContainer = control.$el.find(
                              '.media-widget-fields',
                         );
                         fieldsTemplate = wp.template(
                              'wp-media-widget-image-fields',
                         );
                         fieldsContainer.html(
                              fieldsTemplate(
                                   control.previewTemplateProps.toJSON(),
                              ),
                         );
                    }
               },
               ...
               editMedia: function editMedia() {
                    ...
                    updateCallback = function () {
                         ...
                         mediaProps = mediaFrame
                              .state()
                              .attributes.image.toJSON();
               ...
               getEmbedResetProps: function getEmbedResetProps() {
                    return _.extend(
                         component.MediaWidgetControl.prototype.getEmbedResetProps.call(
                              this,
                         ),
                         {
                              size: 'full',
               ...
               mapModelToPreviewTemplateProps:
                    function mapModelToPreviewTemplateProps() {
                         ...
                         previewTemplateProps.currentFilename = url
                              ? url.replace(/\?.*$/, '').replace(/^.+\//, '')
                              : '';
                         previewTemplateProps.link_url =
                              control.model.get('link_url');
