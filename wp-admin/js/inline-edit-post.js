     ...
     function ($, wp) {
          window.inlineEditPost = {
               ...
               init: function () {
                    ...
                    t.type = $('table.widefat').hasClass('pages')
                         ? 'page'
                         : 'post';
                    // Post ID prefix.
                    t.what = '#post-';
               ...
               setBulk: function () {
                    ...
                    $(
                         '.inline-edit-categories input[name="post_category[]"]',
                    ).each(function () {
                         if (categories[$(this).val()] == checkedPosts.length) {
                              ...
                              if (
                                   !$(this)
                                        .parent()
                                        .find(
                                             'input[name="indeterminate_post_category[]"]',
                                        ).length
                              ) {
                    ...
                    $(
                         '.inline-edit-categories input[name="post_category[]"]:indeterminate',
                    ).on('change', function () {
                    ...
                    $('#bulk-titles .ntdelbutton').click(function () {
                         var $this = $(this),
                              ...
                              $prev = $this
                                   .parent()
                                   .prev()
                                   .children('.ntdelbutton'),
                              $next = $this
                                   .parent()
                                   .next()
                                   .children('.ntdelbutton');

                         $('input#cb-select-all-1, input#cb-select-all-2').prop(
                              'checked',
                              false,
                         );
                         $('table.widefat input[value="' + id + '"]').prop(
                              'checked',
                              false,
                         );
                         $('#_' + id)
               ...
               edit: function (id) {
                    ...
                    if (
                         $(':input[name="post_author"] option', editRow)
                              .length === 1
                    ) {
                    ...
                    for (f = 0; f < fields.length; f++) {
                         ...
                         val = val.text();
                         $(':input[name="' + fields[f] + '"]', editRow).val(
                              val,
                         );
                    }

                    if ($('.comment_status', rowData).text() === 'open') {
                         $('input[name="comment_status"]', editRow).prop(
                              'checked',
                              true,
                         );
                    }
                    if ($('.ping_status', rowData).text() === 'open') {
                         $('input[name="ping_status"]', editRow).prop(
                              'checked',
                              true,
                         );
                    }
                    if ($('.sticky', rowData).text() === 'sticky') {
                         $('input[name="sticky"]', editRow).prop(
                              'checked',
                              true,
                         );
                    }
                    ...
                    if ('future' !== status && Date.now() > post_date) {
                         ...
                         $(
                              'select[name="_status"] option[value="publish"]',
                              editRow,
                         ).remove();
                    }

                    pw = $('.inline-edit-password-input').prop(
                         'disabled',
                         false,
                    );
                    if ('private' === status) {
                         $('input[name="keep_private"]', editRow).prop(
                              'checked',
                              true,
                         );
                         pw.val('').prop('disabled', true);
               ...
               save: function (id) {
                    ...
                    $.post(
                         ...
                         function (r) {
                              ...
                              $('table.widefat .spinner').removeClass(
                                   'is-active',
                              );
               ...
               revert: function () {
                    ...
                    if (id) {
                         ...
                         if ('bulk-edit' === id) {
                              ...
                              $('#' + inlineEditPost.whichBulkButtonId).trigger(
                                   'focus',
                              );
                         } else {
                    ...
                    $('#the-list tr').each(function (i, el) {
                         ...
                         if (locked.hasOwnProperty(key)) {
                              if (!row.hasClass('wp-locked')) {
                                   ...
                                   row.find('.column-title .locked-text').text(
                                        lock_data.text,
                                   );
                                   row.find('.check-column checkbox').prop(
                                        'checked',
                                        false,
                                   );
                                   ...
                                   if (lock_data.avatar_src) {
                                        avatar = $('<img />', {
                                             ...
                                             srcset: lock_data.avatar_src_2x
                                                  ? lock_data.avatar_src_2x +
                                                    ' 2x'
                                                  : undefined,
                                        });
                                        row.find('.column-title .locked-avatar')
                                             .empty()
                                             .append(avatar);
                                   }
