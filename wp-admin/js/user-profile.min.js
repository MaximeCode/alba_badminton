...
!(function (o) {
          ...
          'function' != typeof zxcvbn
               ...
               : (!a.val() || h.hasClass('is-open')
                 ...
                 1 !== parseInt(r.data('start-masked'), 10)
                      ? a.attr('type', 'text')
                      : r.trigger('click'),
                 o('#pw-weak-text-label').text(
                      m('Confirm use of weak password'),
                 ),
                 'mailserver_pass' !== a.prop('id') && o(a).trigger('focus'));
     }
     function g(e) {
          r.attr({ 'aria-label': m(e ? 'Show password' : 'Hide password') })
               .find('.text')
               .text(m(e ? 'Show' : 'Hide'))
               .end()
               .find('.dashicons')
          ...
          r ||
               (r = s.find('.wp-hide-pw')).show().on('click', function () {
                    'password' === a.attr('type')
                         ? (a.attr('type', 'text'), g(!1))
                         : (a.attr('type', 'password'), g(!0));
               });
     }
     function y(e, s, a) {
     ...
     function _() {
          var e = o('#pass1').val();
          if (
               (o('#pass-strength-result').removeClass(
     ...
     function C() {
          ...
          e.length &&
               ...
               o(e).is('.short, .bad')
                    ...
                    : (o(e).is('.empty')
                           ? (l.prop('disabled', !0), i.prop('checked', !1))
                           : l.prop('disabled', !1),
                      n.hide()));
     }
     o(function () {
          ...
          o('#pass1').val('').on('input pwupdate', _),
                         ...
                         if (
                              ((a =
                                   0 === a.length
                                        ...
                                        : a).attr(
                                   'href',
                                   s.children('.css_url').val(),
                              ),
                              'undefined' != typeof wp && wp.svgPainter)
                         ) {
                              try {
                                   e = JSON.parse(
                                        s.children('.icon_colors').val(),
                                   );
                              } catch (e) {}
                              e &&
                                   (wp.svgPainter.setColors(e),
                                   wp.svgPainter.paint());
                         }
                         o.post(ajaxurl, {
                              ...
                              e.success &&
                                   o('body')
                                        .removeClass(e.data.previousScheme)
                                        .addClass(e.data.currentScheme);
                         });
                    }
               }),
               k(),
               o('#generate-reset-link').on('click', function () {
                    var s = o(this),
                         ...
                         e =
                              (s.parent().find('.notice-error').remove(),
                              wp.ajax.post('send-password-reset', e));
                    e.done(function (e) {
                         y(s, !0, e);
                    }),
                         e.fail(function (e) {
          ...
          (window.generatePassword = v),
          o(window).on('beforeunload', function () {
               return !0 === f
                    ? m('Your new password has not been saved.')
                    : u === c.serialize() || w
                      ? void 0
                      : m(
          ...
          o(function () {
               o('.reset-pass-submit').length &&
                    o('.reset-pass-submit button.wp-generate-pw').trigger(
                         'click',
                    );
          });
})(jQuery);
