...
(function ($, wp, settings) {
     ...
     wp.updates.ajaxAlways = function (response) {
          ...
          if (
               ...
               _.map(response.debug, function (message) {
                    ...
                    window.console.log(
                         wp.sanitize.stripTagsAndEncodeText(message),
                    );
               });
          }
     };
     ...
     wp.updates.refreshCount = function () {
          var $adminBarUpdates = $('#wp-admin-bar-updates'),
               ...
               $pluginsNavMenuUpdateCount = $(
                    'a[href="plugins.php"] .update-plugins',
               ),
               $appearanceNavMenuUpdateCount = $(
                    'a[href="themes.php"] .update-plugins',
               ),
               itemCount;
     ...
     wp.updates.decrementCount = function (type) {
          settings.totals.counts.total = Math.max(
               --settings.totals.counts.total,
               0,
          );
     ...
     wp.updates.updatePlugin = function (args) {
          ...
          if ('plugins' === pagenow || 'plugins-network' === pagenow) {
               ...
               $card.removeClass('plugin-card-update-failed')
          ...
          if (
               isPluginInstall &&
               'plugin-information-footer' === $card.attr('id')
          ) {
     ...
     wp.updates.updatePluginError = function (response) {
          ...
          if (
               wp.updates.maybeHandleCredentialError(response, 'update-plugin')
          ) {
               return;
          }
     ...
     wp.updates.installPlugin = function (args) {
          ...
          // Remove previous error messages, if any.
          $card.removeClass('plugin-card-install-failed')
               .find('.notice.notice-error')
               .remove();
     ...
     wp.updates.installPluginError = function (response) {
          ...
          if (
               wp.updates.maybeHandleCredentialError(response, 'install-plugin')
          ) {
               return;
          }
     ...
     wp.updates.checkPluginDependenciesSuccess = function (response) {
          ...
          wp.a11y.speak(
               __('Plugin dependencies check completed successfully.'),
          );
     ...
     wp.updates.activatePlugin = function (args) {
          var $message = $(
               '.plugin-card-' + args.slug + ', #plugin-information-footer',
          ).find('.activate-now, .activating-message');
     ...
     wp.updates.activatePluginSuccess = function (response) {
          ...
          setTimeout(function () {
               $message
                    .removeClass('activated-message')
                    .text(_x('Active', 'plugin'));

               if (
                    'plugin-information-footer' === $message.parent().attr('id')
               ) {
                    wp.updates.setCardButtonStatus({
     ...
     wp.updates.deletePluginSuccess = function (response) {
          ...
          $('[data-plugin="' + response.plugin + '"]')
               ...
               .fadeOut(350, function () {
                    ...
                    if (
                         -1 !==
                         _.indexOf(plugins.recently_activated, response.plugin)
                    ) {
                         ...
                         if (plugins.recently_activated.length) {
                              $views
                                   ...
                                   .text(
                                        '(' +
                                             plugins.recently_activated.length +
                                             ')',
                                   );
                         } else {
                    ...
                    if ($itemsCount.length && $currentView.length) {
                         remainingCount =
                              plugins[$currentView.parent('li').attr('class')]
                                   .length;
                         $itemsCount.text(
                              sprintf(
     ...
     wp.updates.deletePluginError = function (response) {
          ...
          if (response.plugin) {
               $plugin = $(
                    'tr.inactive[data-plugin="' + response.plugin + '"]',
               );
          ...
          if (
               wp.updates.maybeHandleCredentialError(response, 'delete-plugin')
          ) {
               return;
          }
     ...
     wp.updates.updateThemeSuccess = function (response) {
          ...
          wp.updates.addAdminNotice(
               _.extend({ selector: $notice }, updatedMessage),
          );
          wp.a11y.speak(__('Update completed successfully.'));
     ...
     wp.updates.installThemeSuccess = function (response) {
          var $card = $(
                    '.wp-full-overlay-header, [data-slug=' +
                         response.slug +
                         ']',
               ),
               $message;
     ...
     wp.updates.installThemeError = function (response) {
          ...
          if (
               wp.updates.maybeHandleCredentialError(response, 'install-theme')
          ) {
               return;
          }
          ...
          if ('customize' === pagenow) {
               if ($document.find('body').hasClass('modal-open')) {
                    $button = $(
                         '.theme-install[data-slug="' + response.slug + '"]',
                    );
          ...
          if ('themes-network' === pagenow) {
               ...
               $themeRows
                    ...
                    .fadeOut(350, function () {
                         ...
                         $views
                              .find('.all .count')
                              .text('(' + themes.all.length + ')');
                    });
          }

          // DecrementCount from update count.
          if ('themes' === pagenow) {
               var theme = _.find(_wpThemeSettings.themes, {
                    id: response.slug,
               });
               if (theme.hasUpdate) {
                    wp.updates.decrementCount('theme');
     ...
     wp.updates.requestForCredentialsModalClose = function () {
          ...
          if (wp.updates.$elToReturnFocusToFromCredentialsModal) {
               wp.updates.$elToReturnFocusToFromCredentialsModal.trigger(
                    'focus',
               );
          }
     };
     ...
     wp.updates.showErrorInCredentialsForm = function (message) {
          ...
          $filesystemForm
               ...
               .after(
                    '<div class="notice notice-alt notice-error" role="alert"><p>' +
                         message +
                         '</p></div>',
               );
     };
     ...
     $(function () {
          ...
          $filesystemModal.on('submit', 'form', function (event) {
               ...
               wp.updates.filesystemCredentials.ftp.hostname =
                    $('#hostname').val();
               wp.updates.filesystemCredentials.ftp.username =
                    $('#username').val();
               wp.updates.filesystemCredentials.ftp.password =
                    $('#password').val();
               wp.updates.filesystemCredentials.ftp.connectionType = $(
                    'input[name="connection_type"]:checked',
               ).val();
               wp.updates.filesystemCredentials.ssh.publicKey =
                    $('#public_key').val();
               wp.updates.filesystemCredentials.ssh.privateKey =
                    $('#private_key').val();
          ...
          $document.on('credential-modal-cancel', function (event, job) {
               ...
               if ('import' === pagenow) {
                    ...
                    if ('update-plugin' === job.action) {
                         $message = $(
                              'tr[data-plugin="' + job.data.plugin + '"]',
                         ).find('.update-message');
                    } else if ('delete-plugin' === job.action) {
                         $message = $(
                              '[data-plugin="' + job.data.plugin + '"]',
               ...
               if ($message && $message.hasClass('updating-message')) {
                    ...
                    if ('undefined' === typeof originalText) {
                         originalText = $('<p>').html(
                              $message.find('p').data('originaltext'),
                         );
                    }
          ...
          $bulkActionForm.on(
               ...
               function (event) {
                    ...
                    wp.updates.$elToReturnFocusToFromCredentialsModal =
                         $pluginRow.find('.check-column input');
          ...
          $document.on(
               ...
               function (event) {
                    ...
                    wp.updates.$elToReturnFocusToFromCredentialsModal =
                         $themeRow.find('.check-column input');
          ...
          $pluginInstallSearch.on(
               ...
               _.debounce(function (event, eventtype) {
                    ...
                    if (!$searchTab.length) {
                         $searchTab = $(
                              '<li class="plugin-install-search" />',
                         ).append(
                              $('<a />', {
                                   class: 'current',
                                   href: searchLocation,
          ...
          $('#plugin_update_from_iframe').on('click', function (event) {
               ...
               if (
                    ...
                    null === target ||
                    -1 !==
                         window.parent.location.pathname.indexOf(
                              'update-core.php',
                         )
               ) {
                    return;
          ...
          $(window).on('message', function (event) {
               ...
               switch (message.action) {
                    ...
                    case 'update-plugin':
                         if (
                              'undefined' === typeof message.data ||
                              'undefined' === typeof message.data.slug
                         ) {
                              return;
                         }

                         message.data = wp.updates._addCallbacks(
                              message.data,
                              message.action,
                         );
               ...
               function (event) {
                    ...
                    $.post(window.ajaxurl, data)
                         .done(function (response) {
                              ...
                              $document.trigger(
                                   'wp-auto-update-setting-changed',
                                   { state: action, type: type, asset: asset },
                              );
                         })
                         .fail(function () {
                              $parent
