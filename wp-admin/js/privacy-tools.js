...
jQuery(function ($) {
     ...
     function appendResultsAfterRow(
          ...
          if ($requestRow.hasClass('status-request-confirmed')) {
               resultRowClasses =
                    resultRowClasses + ' status-request-confirmed';
          }
     ...
     $('.export-personal-data-handle').on('click', function (event) {
          ...
          function onExportDoneSuccess(zipUrl) {
               ...
               if ('undefined' !== typeof zipUrl) {
                    ...
                    onExportFailure(
                         __('No personal data export file was generated.'),
                    );
               }
               ...
               $.ajax({
                    ...
                    .done(function (response) {
                         ...
                         if (!responseData.done) {
                              setTimeout(
                                   doNextExport(exporterIndex, pageIndex + 1),
                              );
                         } else {
                              setExportProgress(exporterIndex);
                              if (exporterIndex < exportersCount) {
                                   setTimeout(
                                        doNextExport(exporterIndex + 1, 1),
                                   );
                              } else {
                                   setTimeout(function () {
               ...
               $.ajax({
                    ...
                    .done(function (response) {
                         ...
                         if (responseData.items_removed) {
                              hasRemoved =
                                   hasRemoved || responseData.items_removed;
                         }
                         if (responseData.items_retained) {
                              hasRetained =
                                   hasRetained || responseData.items_retained;
                         }
                         ...
                         if (!responseData.done) {
                              setTimeout(
                                   doNextErasure(eraserIndex, pageIndex + 1),
                              );
                         } else {
                              setErasureProgress(eraserIndex);
                              if (eraserIndex < erasersCount) {
                                   setTimeout(
                                        doNextErasure(eraserIndex + 1, 1),
                                   );
                              } else {
                                   setTimeout(function () {
