...
(function ($) {
     ...
     window.setCommentsList = function () {
          ...
          delBefore = function (settings, list) {
               ...
               if (action) {
                    ...
                    a.attr(
                         'data-wp-lists',
                         'delete:the-comment-list:comment-' +
                              id +
                              '::un' +
                              action +
                              '=1',
          ...
          delAfter = function (r, settings) {
               var total_items_i18n,
                    ...
                    animatedCallback,
                    response =
                         true === settings.parsed
                              ? {}
                              : settings.parsed.responses[0],
                    commentStatus =
                         true === settings.parsed
                              ? ''
                              : response.supplemental.status,
                    commentPostId =
                         true === settings.parsed
                              ? ''
                              : response.supplemental.postId,
                    newTotal =
                         true === settings.parsed ? '' : response.supplemental,
                     ...
                     */
                    approved =
                         commentRow.hasClass('approved') &&
                         !commentRow.hasClass('unapproved'),
               ...
               if (targetParent.is('span.undo')) {
                    ...
                    targetParent.is('span.approve:not(.unspam):not(.untrash)')
               ) {
                    approvedDiff = 1;
                    pendingDiff = -1;
                    ...
                    targetParent.is('span.unapprove:not(.unspam):not(.untrash)')
               ) {
                    approvedDiff = -1;
                    pendingDiff = 1;
               ...
               animatedCallback = function () {
                    if (!$('#the-comment-list tr:visible').length) {
                         theList
                              .get(0)
                              .wpList.add(
                                   theExtraList.find('.no-items').clone(),
                              );
                    }
               };
     ...
     window.commentReply = {
          ...
          open: function (comment_id, post_id, action) {
               ...
               if (action == 'edit') {
                    $('#author-name', editRow).val(
                         $('div.author', rowData).text(),
                    );
                    $('#author-email', editRow).val(
                         $('div.author-email', rowData).text(),
                    );
                    $('#author-url', editRow).val(
                         $('div.author-url', rowData).text(),
                    );
                    $('#status', editRow).val(
                         $('div.comment_status', rowData).text(),
                    );
                    $('#replycontent', editRow).val(
                         $('textarea.comment', rowData).val(),
                    );
                    $('#edithead, #editlegend, #savebtn', editRow).show();
                    $(
                         '#replyhead, #replybtn, #addhead, #addbtn',
                         editRow,
                    ).hide();
          ...
          send: function () {
               ...
               if (
                    $('#comment-' + $('#comment_ID').val()).hasClass(
                         'unapproved',
                    )
               )
     ...
     $(function () {
          ...
          if (typeof $.table_hotkeys != 'undefined') {
               ...
               edit_comment = function (event, current_row) {
                    window.location = $('span.edit a', current_row).attr(
                         'href',
                    );
               };
               ...
               $.table_hotkeys(
                    ...
                    {
                         highlight_first:
                              adminCommentsSettings.hotkeys_highlight_first,
                         highlight_last:
                              adminCommentsSettings.hotkeys_highlight_last,
