...
(function ($) {
          ...
          getSliderValue: function (a, b) {
               return isRtl
                    ? this.revisions.length -
                           this.revisions.indexOf(this.get(a)) -
                           1
     ...
     revisions.model.Diffs = Backbone.Collection.extend({
          ...
          getClosestUnloaded: function (ids, centerId) {
               ...
               return _.chain([0].concat(ids))
                    ...
                    .filter(function (diffId) {
                         return (
                              _.isUndefined(self.get(diffId)) &&
                              !self.requests[diffId]
                         );
                    })
                    .value();
          },
          ...
          _loadAll: function (allRevisionIds, centerId, num) {
               var self = this,
                    deferred = $.Deferred(),
                    diffs = _.first(
                         this.getClosestUnloaded(allRevisionIds, centerId),
                         num,
                    );
               if (_.size(diffs) > 0) {
                    this.load(diffs)
     ...
     revisions.view.Tickmarks = wp.Backbone.View.extend({
          ...
          reportTickPosition: function (model, revision) {
               ...
               if (index === this.model.revisions.length - 1) {
                    ...
                    offset = {
                         rightPlusWidth:
                              thisOffset.left - parentOffset.left + 1,
                         leftPlusWidth:
                              thisOffset.right - parentOffset.right + 1,
                    };
               } else {
                    // Normal tick.
                    ...
                    _.extend(offset, {
                         left:
                              offset.left + thisOffset.left - parentOffset.left,
                         right:
                              offset.right +
                              thisOffset.right -
                              parentOffset.right,
     ...
     revisions.view.Tooltip = wp.Backbone.View.extend({
          ...
          toggleVisibility: function () {
               if (this.visible()) {
                    ...
                    this.$el
                         .stop()
                         .fadeTo(this.el.style.opacity * 300, 0, function () {
                              $(this).hide();
                         });
               }
               return;
     ...
     revisions.view.Buttons = wp.Backbone.View.extend({
          ...
          nextRevision: function () {
               var toIndex =
                    this.model.revisions.indexOf(this.model.get('to')) + 1;
     ...
     revisions.view.Slider = wp.Backbone.View.extend({
          ...
          mouseMove: function (e) {
               var zoneCount = this.model.revisions.length - 1, // One fewer zone than models.
                    ...
                    actualX =
                         (isRtl ? $(window).width() - e.pageX : e.pageX) -
                         sliderFrom, // Flipped for RTL - sliderFrom.
                    currentModelIndex = Math.floor(
                         (actualX + tickWidth / 2) / tickWidth,
          ...
          applySliderSettings: function () {
               this.$el.slider(
                    _.pick(this.model.toJSON(), 'value', 'values', 'range'),
               );
               ...
               $(window).on(
                    ...
                    function (e) {
                         var handles,
                              ...
                              sliderRightEdge =
                                   leftDragBoundary + view.$el.width(),
                         ...
                         if (view.model.get('compareTwoMode')) {
                              ...
                              if (handle.is(handles.first())) {
                                   ...
                                   rightDragBoundary = handles
                                        .last()
                                        .offset().left;
                                   rightDragReset =
                                        rightDragBoundary - sliderOffset;
                              } else {
                                   // We're the right handle.
                                   leftDragBoundary =
                                        handles.first().offset().left +
                                        handles.first().width();
                                   leftDragReset =
                                        leftDragBoundary - sliderOffset;
                              }
                         }
          ...
          getPosition: function (position) {
               return isRtl
                    ? this.model.revisions.length - position - 1
                    : position;
          },
          ...
          slide: function (event, ui) {
               ...
               if (this.model.get('compareTwoMode')) {
                    ...
                    attributes = {
                         from: this.model.revisions.at(
                              this.getPosition(ui.values[0]),
                         ),
                         to: this.model.revisions.at(
                              this.getPosition(ui.values[1]),
                         ),
                    };
               } else {
                    attributes = {
                         to: this.model.revisions.at(
                              this.getPosition(ui.value),
                         ),
                    };
                    // If we're at the first revision, unset 'from'.
               ...
               movedRevision = this.model.revisions.at(
                    this.getPosition(ui.value),
               );
     ...
     revisions.Router = Backbone.Router.extend({
          ...
          updateUrl: function () {
               var from = this.model.has('from')
                         ? this.model.get('from').id
                         : 0,
          ...
          state = new revisions.model.FrameState(
               {
                    initialDiffState: {
                         ...
                         compareTwoMode:
                              revisions.settings.compareTwoMode === '1',
                    },
