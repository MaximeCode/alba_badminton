                         ...
                         if (wrap.length && !wrap.is(':visible')) {
                              ...
                              if (received.lock_error.avatar_src) {
                                   avatar = $('<img />', {
                                        ...
                                        srcset: received.lock_error
                                             .avatar_src_2x
                                             ? received.lock_error
                                                    .avatar_src_2x + ' 2x'
                                             : undefined,
                                   });
                                   wrap.find('div.post-locked-avatar')
                                        .empty()
                                        .append(avatar);
                              }
...
(function ($) {
     ...
     $(function () {
          ...
          .on('heartbeat-send.wp-refresh-nonces', function (e, data) {
               ...
               if (
                    check ||
                    ($authCheck.length && !$authCheck.hasClass('hidden'))
               ) {
                    if (
                         (post_id = $('#post_ID').val()) &&
                         $('#_wpnonce').val()
...
jQuery(function ($) {
     ...
     $submitButtons = $submitpost
          ...
          .on('click.edit-post', function (event) {
               ...
               if (
                    $button.hasClass('submitdelete') ||
                    $button.is('#post-preview')
               ) {
                    return;
               }
     ...
     $('.categorydiv').each(function () {
          ...
          catAddBefore = function (s) {
               ...
               s.data +=
                    '&' +
                    $(':checked', '#' + taxonomy + 'checklist').serialize();
     ...
     if ($('#submitdiv').length) {
          ...
          updateVisibility = function () {
               ...
               if (
                    $postVisibilitySelect.find('input:radio:checked').val() !=
                    'public'
               ) {
          ...
          $postVisibilitySelect
               ...
               .on('click', function (event) {
                    ...
                    $('#sticky').prop(
                         'checked',
                         $('#hidden-post-sticky').prop('checked'),
                    );
               ...
               .on('click', function (event) {
                    if ($timestampdiv.is(':hidden')) {
                         $timestampdiv.slideDown('fast', function () {
                              $(
                                   'input, select',
                                   $timestampdiv.find('.timestamp-wrap'),
                              )
                                   .first()
                                   .trigger('focus');
                         });
          ...
          function endDrag() {
               ...
               if (mce) {
                    ...
                    toolbarHeight = parseInt(
                         $(
                              '#wp-content-editor-container .mce-toolbar-grp',
                         ).height(),
                         10,
                    );
                    ...
                    height =
                         parseInt($('#content_ifr').css('height'), 10) +
                         toolbarHeight -
                         28;
               } else {
                    $textarea.trigger('focus');
