...
wp.customize.selectiveRefresh = (function ($, api) {
          ...
          /** @lends wp.customize.SelectiveRefresh.Partial.prototype */ {
               ...
               ready: function () {
                    ...
                    _.each(partial.placements(), function (placement) {
                         $(placement.container).attr(
                              'title',
                              self.data.l10n.shiftClickToEdit,
                         );
                         partial.createEditShortcutForPlacement(placement);
                    });
                    $(document).on(
                         'click',
                         partial.params.selector,
                         function (e) {
                              if (!e.shiftKey) {
                                   return;
                              }
               ...
               createEditShortcutForPlacement: function (placement) {
                    ...
                    if (
                         ...
                         $placementContainer.closest(illegalAncestorSelector)
                              .length
                    ) {
                         return;
                    }
               ...
               placements: function () {
                    ...
                    selector +=
                         '[data-customize-partial-id="' + partial.id + '"]'; // @todo Consider injecting customize-partial-id-${id} classnames instead.
               ...
               settings: function () {
                    ...
                    if (
                         ...
                         0 !== partial.params.settings.length
                    ) {
               ...
               showControl: function () {
                    ...
                    if (partial.getType() === 'nav_menu') {
                         if (partial.params.navMenuArgs.theme_location) {
                              ...
                              settingId =
                                   'nav_menu[' +
                                   String(partial.params.navMenuArgs.menu) +
                                   ']';
                         }
                    }
               ...
               renderContent: function (placement) {
                    var partial = this,
                         ...
                         newContainerElement;
                    if (!placement.container) {
                         partial.fallback(new Error('no_container'), [
                              placement,
                         ]);
                         return false;
                    }
                    ...
                    if (false === placement.addedContent) {
                         partial.fallback(new Error('missing_render'), [
                              placement,
                         ]);
                         return false;
                    }
                    ...
                    if (!_.isString(placement.addedContent)) {
                         partial.fallback(new Error('non_string_content'), [
                              placement,
                         ]);
                         return false;
                    }
                    ...
                    try {
                         ...
                         if (partial.params.containerInclusive) {
                              ...
                              placement.removedNodes.replaceWith(
                                   placement.container,
                              );
                              placement.container.attr(
                                   'title',
                         ...
                         placement.container.removeClass(
                    ...
                    placement.container.data(
                         'customize-partial-content-rendered',
                         true,
                    );
     ...
     self.requestPartial = function (partial) {
          ...
          if (
               !partialRequest ||
               'pending' !== partialRequest.deferred.state()
          ) {
               partialRequest = {
                    deferred: $.Deferred(),
                    partial: partial,
          ...
          self._debouncedTimeoutId = setTimeout(function () {
               ...
               _.each(
                    self._pendingPartialRequests,
                    function (pending, partialId) {
                         partialsPlacements[partialId] =
                              pending.partial.placements();
                         ...
                         function (pending, partialId) {
                              ...
                              if (!_.isArray(data.contents[partialId])) {
                                   ...
                                   placementsContents = _.map(
                                        data.contents[partialId],
                                        function (content, i) {
                                             var partialPlacement =
                                                  partialsPlacements[partialId][
                                                       i
                                                  ];
                                             if (partialPlacement) {
                                                  partialPlacement.addedContent =
                                                       content;
                                             } else {
                                                  partialPlacement =
                                                       new Placement({
     ...
     api.bind('preview-ready', function () {
          ...
          _.each(self.data.partials, function (data, id) {
               ...
               if (!partial) {
                    Constructor =
                         self.partialConstructor[data.type] || self.Partial;
