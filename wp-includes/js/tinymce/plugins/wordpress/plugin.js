     ...
     tinymce.PluginManager.add('wordpress', function (editor) {
          ...
          function toggleToolbars(state) {
               ...
               if (editor.theme.panel) {
                    toolbars = editor.theme.panel.find(
                         '.toolbar:not(.menubar)',
                    );
               }
                    ...
                    event.content = event.content.replace(
                         ...
                         function (image) {
                              ...
                              if (image.indexOf('data-wp-more="more"') !== -1) {
                                   ...
                                   image.indexOf('data-wp-more="nextpage"') !==
                                   -1
                              ) {
                                   string = '<!--nextpage-->';
                              } else if (
                                   image.indexOf('data-wp-preserve') !== -1
                              ) {
                                   if (
                                        (match = image.match(
                                             / data-wp-preserve="([^"]+)"/,
          ...
          editor.addCommand('WP_More', function (tag) {
               ...
               if (parent) {
                    if (parent.nodeName === 'P') {
                         parent.appendChild(
                              dom.create('p', null, html).firstChild,
                         );
                    } else {
          ...
          editor.on('init', function () {
               ...
               if ($) {
                    ...
                    $(function () {
                         $(document).triggerHandler('tinymce-editor-init', [
                              editor,
                         ]);
                    });
               }
          ...
          editor.on('beforerenderui', function () {
               if (editor.theme.panel) {
                    each(
                         ...
                         function (buttonType) {
                              replaceButtonsTooltips(
                                   editor.theme.panel.find(buttonType),
                              );
                         },
                    );
               ...
               each(editor.theme.panel.find('listbox'), function (listbox) {
                    if (listbox && listbox.settings.text === 'Paragraph') {
                         each(listbox.settings.values, function (item) {
                              if (
                                   item.text &&
                                   wpTooltips.hasOwnProperty(item.text)
                              ) {
                                   item.shortcut =
                                        '(' + wpTooltips[item.text] + ')';
                              }
                         });
                    }
               });
          ...
          editor.on(
               ...
               function () {
                    var Factory = tinymce.ui.Factory,
                         ...
                         mceIframe = document.getElementById(
                              editor.id + '_ifr',
                         ),
                         mceToolbar,
                         mceStatusbar,
                    ...
                    if (editor.id === 'content') {
                         wpStatusbar =
                              document.getElementById('post-status-info');
                    }
                    ...
                    function create(buttons, bottom) {
                         ...
                         function reposition() {
                              ...
                              if (
                                   spaceTop >= editorHeight ||
                                   spaceBottom >= editorHeight
                              ) {
                                   this.scrolling = true;
                                   this.hide();
                              ...
                              if (
                                   selection.left < 0 ||
                                   selection.right > iframeRect.width
                              ) {
                                   left =
                                        iframeRect.left +
                                        scrollX +
