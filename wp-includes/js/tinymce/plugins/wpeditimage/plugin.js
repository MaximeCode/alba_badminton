...
tinymce.PluginManager.add('wpeditimage', function (editor) {
               ...
               editor.addButton('wp_img_' + name, {
                    ...
                    onPostRender: function () {
                         ...
                         editor.on('NodeChange', function (event) {
                              ...
                              if ('alignnone' === name) {
                                   self.active(
                                        !/\balign(left|center|right)\b/.test(
                                             node.className,
                                        ),
                                   );
                              } else {
               ...
               function (all, dl) {
                    ...
                    out = dl.replace(
                         ...
                         function (a, b, c, caption) {
                              ...
                              align =
                                   classes.match(/align[a-z]+/i) || 'alignnone';
     ...
     function extractImageData(imageNode) {
          ...
          tinymce.each(classes, function (name) {
               if (/^wp-image/.test(name)) {
                    metadata.attachment_id = parseInt(
                         name.replace('wp-image-', ''),
                         10,
                    );
     ...
     function hasTextContent(node) {
          return (
               node &&
               !!(node.textContent || node.innerText).replace(/\ufeff/g, '')
          );
     }
     ...
     function updateImage($imageNode, imageData) {
          ...
          if (imageData.caption) {
               ...
               if (imageData.captionClassName) {
                    className +=
                         ' ' +
                         imageData.captionClassName.replace(/[<>&]+/g, '');
               }
          ...
          if (
               ...
               node.className = trim(
                    node.className.replace(
                         / ?align(left|center|right|none)/g,
                         '',
                    ) + replacement,
               );
