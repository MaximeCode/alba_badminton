...
var tinyMCEPopup = {
     ...
     init: function () {
          ...
          if (settings.popup_css_add) {
               settings.popup_css +=
                    ',' +
                    self.editor.documentBaseURI.toAbsolute(
    ...
    setTimeout(function() {
     ...
     restoreSelection: function () {
          ...
          if (!self.isWindow && tinymce.isIE) {
               self.editor.selection.moveToBookmark(
                    self.editor.windowManager.bookmark,
               );
          }
     },
     ...
     requireLangPack: function () {
          var self = this,
               url =
                    self.getWindowArg('plugin_url') ||
                    self.getWindowArg('theme_url'),
               settings = self.editor.settings,
               lang;
     ...
     _onDOMLoaded: function () {
          ...
          if (t.features.translate_i18n !== false) {
               ...
               var langCode =
                    (tinymce.settings ? tinymce.settings : t.editor.settings)
                         .language || 'en';
               for (var key in map) {
                    tinymce.i18n.data[langCode + '.' + key] =
                         tinymce.i18n.translate(map[key]);
               }
          ...
          if (
               !t.editor.getParam('browser_preferred_colors', false) ||
               !t.isWindow
          ) {
