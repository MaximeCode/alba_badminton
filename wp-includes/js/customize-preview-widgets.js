...
wp.customize.widgetsPreview = wp.customize.WidgetCustomizerPreview = (function (
     ...
     self.WidgetPartial = api.selectiveRefresh.Partial.extend(
          /** @lends wp.customize.widgetsPreview.WidgetPartial.prototype */ {
               ...
               initialize: function (id, options) {
                    ...
                    partial.widgetIdParts = self.parseWidgetId(
                         partial.widgetId,
                    );
                    options = options || {};
                    options.params = _.extend(
                         {
                              settings: [
                                   self.getWidgetSettingId(partial.widgetId),
                              ],
               ...
               refresh: function () {
                    var partial = this,
                         refreshDeferred;
                    if (
                         !self.selectiveRefreshableWidgets[
                              partial.widgetIdParts.idBase
                         ]
                    ) {
                         ...
                         return api.selectiveRefresh.Partial.prototype.refresh.call(
                              partial,
                         );
                    }
               },
          ...
          /** @lends wp.customize.widgetsPreview.SidebarPartial.prototype */ {
               ...
               initialize: function (id, options) {
                    ...
                    if (!partial.params.sidebarArgs) {
                         throw new Error(
                              'The sidebarArgs param was not provided.',
                         );
                    }
               ...
               ready: function () {
                    ...
                    _.each(sidebarPartial.settings(), function (settingId) {
                         api(settingId).bind(
                              _.bind(
                                   sidebarPartial.handleSettingChange,
                                   sidebarPartial,
                              ),
                         );
                    });
                    ...
                    api.bind('change', function (widgetSetting) {
                         ...
                         if (
                              -1 !==
                              _.indexOf(sidebarPartial.getWidgetIds(), widgetId)
                         ) {
                              sidebarPartial.ensureWidgetPlacementContainers(
                                   widgetId,
                              );
                         }
                    });
               ...
               findDynamicSidebarBoundaryNodes: function () {
                    ...
                    regExp =
                         /^(dynamic_sidebar_before|dynamic_sidebar_after):(.+):(\d+)$/;
                    recursiveCommentTraversal = function (childNodes) {
                         _.each(childNodes, function (node) {
                              ...
                              if (8 === node.nodeType) {
                                   ...
                                   if (
                                        !matches ||
                                        matches[2] !== partial.sidebarId
                                   ) {
                                        return;
                                   }
                                   if (
                                        _.isUndefined(boundaryNodes[matches[3]])
                                   ) {
                                        boundaryNodes[matches[3]] = {
                                             before: null,
                                             after: null,
                                             instanceNumber: parseInt(
                                                  matches[3],
                                                  10,
                                             ),
                                        };
                                   ...
                                   if (
                                        'dynamic_sidebar_before' === matches[1]
                                   ) {
               ...
               placements: function () {
                    var partial = this;
                    return _.map(
                         partial.findDynamicSidebarBoundaryNodes(),
               ...
               getWidgetIds: function () {
                    ...
                    if (!_.isArray(widgetIds)) {
                         throw new Error(
                              'Expected setting to be array of widget IDs',
                         );
                    }
               ...
               reflowWidgets: function () {
                    ...
                    _.each(sidebarPlacements, function (sidebarPlacement) {
                         ...
                         _.each(widgetPartials, function (widgetPartial) {
                              _.each(
                                   widgetPartial.placements(),
                                   function (widgetPlacement) {
                                        if (
                                             sidebarPlacement.context
                                                  .instanceNumber ===
                                             widgetPlacement.context
                                                  .sidebar_instance_number
                                        ) {
                                             thisPosition =
                                                  widgetPlacement.container.index();
                                             sidebarWidgets.push({
                         ...
                         function (sidebarPlacement) {
                              ...
                              widgetContainerElement = $(
                                   sidebarPartial.params.sidebarArgs.before_widget
                                        ...
                                        sidebarPartial.params.sidebarArgs
                                             .after_widget,
                              );
               ...
               handleSettingChange: function (newWidgetIds, oldWidgetIds) {
                    ...
                    needsRefresh =
                         (oldWidgetIds.length > 0 &&
                              0 === newWidgetIds.length) ||
     ...
     self.highlightWidget = function (widgetId) {
          ...
          $body.find('.widget-customizer-highlighted-widget').removeClass(
