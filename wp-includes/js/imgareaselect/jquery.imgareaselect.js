...
(function ($) {
     ...
     $.imgAreaSelect = function (img, options) {
          ...
          function adjust() {
                ...
                */
               imgOfs = {
                    left: floor($img.offset().left),
                    top: floor($img.offset().top),
               };
               ...
               maxWidth = floor(
                    min(options.maxWidth / scaleX || 1 << 24, imgWidth),
               );
               maxHeight = floor(
                    min(options.maxHeight / scaleY || 1 << 24, imgHeight),
               );
               ...
               parOfs = /absolute|relative/.test($parent.css('position'))
                    ? {
                           left:
                                floor($parent.offset().left) -
                                $parent.scrollLeft(),
                           top:
                                floor($parent.offset().top) -
                                $parent.scrollTop(),
                      }
                    : position == 'fixed'
                      ? {
          ...
          function update(resetKeyPress) {
               ...
               $border
                    .width(
                         max(
                              w - $border.outerWidth() + $border.innerWidth(),
                              0,
                         ),
                    )
                    .height(
                         max(
                              h - $border.outerHeight() + $border.innerHeight(),
                              0,
                         ),
                    );
          ...
          function selectingMouseMove(event) {
               x2 =
                    /w|e|^$/.test(resize) || aspectRatio
                         ? evX(event)
                         : viewX(selection.x2);
               y2 =
                    /n|s|^$/.test(resize) || aspectRatio
                         ? evY(event)
                         : viewY(selection.y2);
          ...
          function setOptions(newOptions) {
               ...
               if (newOptions.handles != null) {
                    ...
                    i = newOptions.handles
                         ? newOptions.handles == 'corners'
                              ? 4
                              : 8
                         : 0;
               ...
               if (msie) {
                    if (
                         (o = ($outer.css('filter') || '').match(
                              /opacity=(\d+)/,
                         ))
                    )
                         $outer.css('opacity', o[1] / 100);
                    if (
                         (o = ($border.css('filter') || '').match(
                              /opacity=(\d+)/,
                         ))
                    )
               ...
               aspectRatio =
                    (d = (options.aspectRatio || '').split(/:/))[0] / d[1];
     ...
     $.fn.imgAreaSelect = function (options) {
          ...
          this.each(function () {
               ...
               if ($(this).data('imgAreaSelect')) {
                    ...
                    $(this).data(
                         'imgAreaSelect',
                         new $.imgAreaSelect(this, options),
                    );
               }
          });
