...
(function (window, wp, shortcode, $) {
     ...
     wp.mce.views = {
          ...
          register: function (type, extend) {
               views[type] = wp.mce.View.extend(
                    _.extend(extend, { type: type }),
               );
          },
          ...
          setMarkers: function (content, editor) {
               ...
               _.each(views, function (view, type) {
                    ...
                    _.each(current, function (piece) {
                         ...
                         while (
                              remaining &&
                              (result = view.prototype.match(remaining))
                         ) {
                              ...
                              remaining = remaining.slice(
                                   result.index + result.content.length,
                              );
                         }
...
(function (window, views, media, $) {
     ...
     function verifyHTML(string) {
          ...
          parser =
               parser || new window.tinymce.html.DomParser(settings, schema);
          serializer =
               serializer ||
               new window.tinymce.html.Serializer(settings, schema);
          ...
          edit: function (text, update) {
               ...
               _.each(this.state, function (state) {
                    frame.state(state).on('update', function (selection) {
                         update(
                              media[type].shortcode(selection).string(),
                              type === 'gallery',
                         );
                    });
               });
     ...
     views.register(
          ...
          _.extend({}, embed, {
               match: function (content) {
                    ...
                    var re =
                         /(^|<p>(?:<span data-mce-type="bookmark"[^>]+>\s*<\/span>)?)(https?:\/\/[^\s"]+?)((?:<span data-mce-type="bookmark"[^>]+>\s*<\/span>)?<\/p>\s*|$)/gi;
